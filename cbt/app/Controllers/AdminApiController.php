...
final class AdminApiController
{
    ...
    public function sync(): void
    {
        ...
        try{
            ...
            $q=$db->prepare('SELECT id FROM tests WHERE external_id=? FOR UPDATE');$q->execute([(string)$payload['external_id']]);$testId=(int)$q->fetchColumn();
            ...
            if($testId){$q=$db->prepare('SELECT COUNT(*) FROM attempts WHERE test_id=?');$q->execute([$testId]);if((int)$q->fetchColumn()>0)throw new HttpException(409,'A test with attempts cannot have its question set replaced.');$db->prepare('UPDATE tests SET title=?,exam_name=?,category=?,subject_id=?,full_marks=?,total_questions=?,duration_minutes=?,correct_mark=?,negative_mark=?,starts_at=?,ends_at=?,fee=?,status=?,published_at=?,updated_at=NOW(),deleted_at=NULL WHERE id=?')->execute([...$values,$testId]);$db->prepare('DELETE FROM questions WHERE test_id=?')->execute([$testId]);}
            else{$key=strtoupper(substr(hash('sha256',(string)$payload['external_id']),0,8));$db->prepare('INSERT INTO tests(external_id,unique_key,title,exam_name,category,subject_id,full_marks,total_questions,duration_minutes,correct_mark,negative_mark,starts_at,ends_at,fee,status,created_by,published_at,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW(),NOW())')->execute([(string)$payload['external_id'],$key,...array_slice($values,0,13),$adminId,$published]);$testId=(int)$db->lastInsertId();}
            ...
            $db->commit();Response::json(['ok'=>true,'test_id'=>$testId,'external_id'=>$payload['external_id']]);
